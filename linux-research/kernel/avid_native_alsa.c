...
/* Experimental ALSA playback for the exact HD Native/192 setup already
 * tested with avid_native_dma_probe. No firmware or clock programming.
 * DMA setup/stop copied unchanged from the first audible milestone.
 */
...
#define DRV "avid_native_alsa"
static bool enable_experimental;
module_param(enable_experimental, bool, 0400);
static uint max_stream_seconds = 30;
module_param(max_stream_seconds, uint, 0400);
MODULE_PARM_DESC(max_stream_seconds, "Session limit, 30..360 seconds; default 30, explicit 360 for stability test");
...
static int native_open(struct snd_pcm_substream *ss)
{
    ...
    int err = 0;
    /* Prevent desktop auto-probing/playback during this manual prototype. */
...
static int native_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    err = snd_card_new(&pdev->dev, -1, "AvidNative", THIS_MODULE, 0, &e->card);
    if (err)
        goto fail;
    strscpy(e->card->driver, "AvidNative");
    strscpy(e->card->shortname, "HD Native + 192 experimental");
    snprintf(e->card->longname, sizeof(e->card->longname), "HD Native PCIe + 192 at %s (experimental playback)", pci_name(pdev));
    ...
    if (err)
        goto fail;
    e->worker = kthread_run(native_poll, e, "avid-native-pcm");
    ...
    bound = true;
    dev_info(&pdev->dev, "ALSA ready: card=%d id=%s; 48 kHz S24_3LE stereo, %u s stream limit; hardware playback still experimental\n",
...
/* No MODULE_DEVICE_TABLE; manual loading only, never installed by our script. */
...
{
    int err;
    if (!enable_experimental || max_stream_seconds < 30 || max_stream_seconds > 360)
        return -EINVAL;
    pr_info(DRV ": begin manual ALSA experiment\n");
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Experimental HD Native PCIe + 192 ALSA playback, 48 kHz stereo, manual opt-in");
