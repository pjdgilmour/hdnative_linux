...
#define DRV "avid_native_capture"
...
static int native_prepare(struct snd_pcm_substream *ss)
{
    ...
    if (err)
        goto cleanup;
    e->prepared = true;
    dev_info(&e->pdev->dev, "prepared: stereo S24_3LE 48000 Hz buffer=4096 period=1024; route41=9 route4d=1 control0=0\n");
    goto out;
cleanup:
    finish_session(e);
out:
...
static int native_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    err = snd_card_new(&pdev->dev, -1, "AvidCapture", THIS_MODULE, 0, &e->card);
    if (err)
        goto fail;
    strscpy(e->card->driver, "AvidCaptureTest");
    strscpy(e->card->shortname, "Avid 192 capture experiment");
    snprintf(e->card->longname, sizeof(e->card->longname), "HD Native PCIe + 192 at %s (capture only, experimental)", pci_name(pdev));
    err = snd_pcm_new(e->card, "192 analog 1-2", 0, 0, 1, &e->pcm);
    if (err)
        goto fail;
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("HD Native PCIe + 192 bounded ALSA capture experiment, 48 kHz stereo");
