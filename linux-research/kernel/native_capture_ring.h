...
#ifndef NATIVE_CAPTURE_RING_H
#define NATIVE_CAPTURE_RING_H
                   ...
                   unsigned char *pcm, unsigned int hw)
{
    ...
    if (r->produced - r->taken + n >= NC_ALSA_FRAMES)
        return -EPIPE;
    for (i = 0; i < n; i++) {
        const unsigned char *src = rx + ((r->hw + i) % NC_RX_FRAMES) * 256 + 4;
                       ...
                       unsigned int count, int tone)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        for (ch = 0; ch < 2; ch++) {
            ...
            dst[4 + ch*3] = value & 255;
            dst[5 + ch*3] = (value >> 8) & 255;
            dst[6 + ch*3] = (value >> 16) & 255;
        }
    }
}
#endif
