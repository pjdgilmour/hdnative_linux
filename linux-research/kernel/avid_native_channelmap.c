...
#define DRV "avid_native_channelmap"
...
MODULE_PARM_DESC(input_pair, "Analog input pair 1..4 (physical 1-2..7-8)");
MODULE_PARM_DESC(output_pair, "Analog output pair 1..8 (physical 1-2..15-16)");
static bool loopback;
module_param(loopback, bool, 0400);
MODULE_PARM_DESC(loopback, "Emit -40 dBFS test tones for a physical analog loopback; otherwise TX silence");
...
static int native_prepare(struct snd_pcm_substream *ss)
{
    ...
    if (err)
        goto cleanup;
    e->prepared = true;
    dev_info(&e->pdev->dev, "prepared: stereo S24_3LE 48000 Hz buffer=4096 period=1024; route41=%u route%02x=1 input_pair=%u output_pair=%u control0=0\n",
             8 + input_pair, n192_output_reg(&e->peripheral), input_pair, output_pair);
    goto out;
cleanup:
    finish_session(e);
out:
...
static int native_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    err = snd_card_new(&pdev->dev, -1, "AvidChannelMap", THIS_MODULE, 0, &e->card);
    if (err)
        goto fail;
    strscpy(e->card->driver, "AvidChannelMapTest");
    strscpy(e->card->shortname, "Avid 192 analog pair test");
    snprintf(e->card->longname, sizeof(e->card->longname), "HD Native PCIe + 192 at %s (capture only, experimental)", pci_name(pdev));
    err = snd_pcm_new(e->card, "192 selected analog pair", 0, 0, 1, &e->pcm);
    if (err)
        goto fail;
...
static int __init native_init(void)
{
    ...
    if (!enable_experimental || max_stream_seconds != 15 ||
        input_pair < 1 || input_pair > 4 || output_pair < 1 || output_pair > 8)
        return -EINVAL;
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("HD Native PCIe + 192 bounded ALSA analog pair mapping, 48 kHz stereo");
