...
/* Bounded capture experiment derived from validated desktop playback.
 * Root-only capture, 15 second watchdog, S24_3LE stereo at 48 kHz.
 * Optional independent TX tones for physical loopback; no input monitoring.
...
#define DRV "avid_native_adat"
module_param(windows_state, bool, 0400);
MODULE_PARM_DESC(windows_state, "Use observed Windows enclosure routes on logical 9-16; no peripheral writes");
static bool enable_experimental;
static bool allow_idle_dma;
module_param(allow_idle_dma, bool, 0400);
MODULE_PARM_DESC(allow_idle_dma, "Opt-in: accept stable aligned residual DMA addresses only with engines and PCI mastering off");
module_param(enable_experimental, bool, 0400);
static uint max_stream_seconds = 15;
static unsigned int optical_path;
module_param(optical_path, uint, 0400);
MODULE_PARM_DESC(optical_path, "1=enclosure, 2=digital module; retains existing optical format");
module_param(input_pair, uint, 0400);
module_param(output_pair, uint, 0400);
MODULE_PARM_DESC(input_pair, "Optical input pair 1..4 (physical 1-2..7-8)");
MODULE_PARM_DESC(output_pair, "Optical output pair 1..4 (physical 1-2..7-8)");
static bool loopback;
module_param(loopback, bool, 0400);
MODULE_PARM_DESC(loopback, "Emit -40 dBFS test tones for a physical optical loopback; otherwise TX silence");
module_param(max_stream_seconds, uint, 0400);
MODULE_PARM_DESC(max_stream_seconds, "Capture limit 15 seconds; root-only test module");
...
static unsigned long long total_frames, last_frames, ring_wraps, tx_frames, max_poll_us;
module_param(total_frames, ullong, 0444);
module_param(last_frames, ullong, 0444);
module_param(ring_wraps, ullong, 0444);
module_param(tx_frames, ullong, 0444);
...
struct native_alsa {
    ...
    struct native_capture_ring ring;
    u32 tx_hw;
    u64 tx_queued;
    u64 notified;
    ktime_t last_poll, last_progress, last_tx_progress, started;
};
...
/* Residual address values are opaque snapshots, never CPU/DMA pointers
 * to be reused. Only Linux dma_alloc_coherent addresses reach a START. */
static int quiet_state(struct native_alsa *e)
{
    ...
    unsigned int pass, i;
    bool residual = false;
    if (pci_read_config_word(e->pdev, PCI_COMMAND, &command)) return -EIO;
    if (command & PCI_COMMAND_MASTER) return -EBUSY;
    if (!pci_wait_for_pending_transaction(e->pdev)) return -EBUSY;
    for (pass = 0; pass < 2; pass++) {
        ...
        if (rd(e, CTL) != 0xa00 || rd(e, RESET) || rd(e, DL_CTL) ||
            ...
            return -EBUSY;
        for (i = 0; i < 6; i++) {
            u32 value = rd(e, saved_offsets[7 + i]);
            if (pass && value != addresses[i]) return -EBUSY;
            addresses[i] = value;
            if (value) residual = true;
        }
        if (!pass) usleep_range(1000, 1500);
    }
    if (pci_read_config_word(e->pdev, PCI_COMMAND, &command)) return -EIO;
    if (command & PCI_COMMAND_MASTER) return -EBUSY;
    if (residual) {
        if (!allow_idle_dma) return -EBUSY;
        /* Narrowly support the observed 32-bit, 256-byte-aligned shape.
         * Never map, dereference or enable DMA with these old addresses. */
        for (i = 0; i < 6; i += 2)
            if (!addresses[i] || (addresses[i] & 255) || addresses[i + 1])
                return -EBUSY;
        dev_info(&e->pdev->dev,
                 "idle DMA handoff: rx=%08x tx=%08x status=%08x; engines/master off, pending transactions drained; addresses will be replaced before START\n",
                 addresses[0], addresses[2], addresses[4]);
    }
    return 0;
}

static int verify_dma_buffers(struct native_alsa *e)
{
    u16 command;
    if (pci_read_config_word(e->pdev, PCI_COMMAND, &command)) return -EIO;
    if (command & PCI_COMMAND_MASTER) return -EBUSY;
    if (rd(e, RESET) || rd(e, DL_CTL) || rd(e, DL_IRQ_MASK) ||
        (rd(e, DMA_CTL) & DMA_ENABLE) || (rd(e, CTL) & STREAM_ENABLE))
        return -EBUSY;
    if (rd(e, 0x40918) != lower_32_bits(e->rx_dma) ||
        rd(e, 0x40958) != upper_32_bits(e->rx_dma) ||
        rd(e, 0x41118) != lower_32_bits(e->tx_dma) ||
        rd(e, 0x41158) != upper_32_bits(e->tx_dma) ||
        rd(e, 0x4093c) != lower_32_bits(e->status_dma) ||
        rd(e, 0x4097c) != upper_32_bits(e->status_dma))
        return -EIO;
    return 0;
}
...
static int finish_session(struct native_alsa *e)
{
    ...
    if (e->quarantined)
        return -EBUSY;
    /* ALSA may free/close before any prepare, and again after STOP.
    ...
    if (had_session) {
        ...
        dev_info(&e->pdev->dev,
                 ...
                 e->ring.taken, e->ring.produced, e->ring.hw,
...
static const struct snd_pcm_hardware native_hardware = {
    ...
    .buffer_bytes_max = NC_ALSA_FRAMES * 6U,
    .period_bytes_min = NC_PERIOD * 6U,
    .period_bytes_max = NC_PERIOD * 6U,
...
static int native_open(struct snd_pcm_substream *ss)
{
    ...
    int err = 0;
    /* First capture experiment is only opened by the root test runner. */
...
static int native_hw_free(struct snd_pcm_substream *ss)
{
    ...
    if (err)
        dev_err(&e->pdev->dev, "hw_free cleanup failed=%d reg=%02x expected=%x observed=%x\n",
                err, e->peripheral.check_reg, e->peripheral.check_expected, e->peripheral.check_observed);
    mutex_unlock(&e->io_lock);
    return err;
}
...
static int native_prepare(struct snd_pcm_substream *ss)
{
    ...
    unsigned int i;
    int err;
    const char *stage = "cleanup-before-prepare";
    mutex_lock(&e->io_lock);
    err = finish_session(e);
    ...
    if (r->format != SNDRV_PCM_FORMAT_S24_3LE || r->rate != 48000 ||
        r->channels != 2 || r->buffer_size != NC_ALSA_FRAMES ||
        r->period_size != NC_PERIOD || !r->dma_area) {
        err = -EINVAL;
        goto out;
    }
    stage = "quiet-state";
    ...
    e->tx_hw = 0;
    e->tx_queued = NC_ALSA_FRAMES;
    e->notified = 0;
    memset(e->tx, 0, TX_BYTES);
    nc_tx_fill(e->tx, 0, NC_ALSA_FRAMES, loopback);
    ...
    e->configured = true;
    setup_dma(e);
    stage = "dma-buffer-readback";
    err = verify_dma_buffers(e);
    if (err) goto cleanup;
    stage = "peripheral-control-readback";
    err = n192_check_unmuted(&e->peripheral);
    if (!err) {
        stage = "peripheral-route-readback";
    ...
    goto out;
cleanup:
    dev_err(&e->pdev->dev, "optical prepare error=%d; last check reg=%02x expected=%x observed=%x (ffffffff=unavailable)\n",
            err, e->peripheral.check_reg, e->peripheral.check_expected, e->peripheral.check_observed);
    finish_session(e);
out:
    if (err) {
        u16 command = 0;
        int config_err = pci_read_config_word(e->pdev, PCI_COMMAND, &command);
        last_error = err;
        dev_err(&e->pdev->dev, "DigiLink idle waits=%u last_tx=%08x last_rx=%08x\n",
                e->peripheral.idle_waits, e->peripheral.idle_tx, e->peripheral.idle_rx);
        dev_err(&e->pdev->dev,
                "prepare refused: stage=%s error=%d configured=%u faulted=%u peripheral_reg=%02x expected=%x observed=%x\n",
                stage, err, e->configured, e->faulted, e->peripheral.check_reg,
                e->peripheral.check_expected, e->peripheral.check_observed);
        dev_err(&e->pdev->dev,
                "prepare state: pci_read=%d command=%04x ctl=%08x reset=%08x dlctl=%08x dma=%08x irq=%08x dmairq=%08x txcmd=%08x rxhdr=%08x\n",
                config_err, command, rd(e, CTL), rd(e, RESET), rd(e, DL_CTL),
                rd(e, DMA_CTL), rd(e, DL_IRQ_MASK), rd(e, 0x42008),
                rd(e, 0x7001c), rd(e, 0x70040));
        dev_err(&e->pdev->dev,
                "prepare addresses: rx=%08x:%08x tx=%08x:%08x status=%08x:%08x\n",
                rd(e, 0x40958), rd(e, 0x40918), rd(e, 0x41158), rd(e, 0x41118),
                rd(e, 0x4097c), rd(e, 0x4093c));
    }
    mutex_unlock(&e->io_lock);
    return err;
}
...
static int native_ack(struct snd_pcm_substream *ss)
{
    ...
    err = nc_ack(&e->ring, READ_ONCE(r->control->appl_ptr), r->boundary);
out:
    if (err)
        last_error = err;
    mutex_unlock(&e->io_lock);
...
static int native_trigger(struct snd_pcm_substream *ss, int cmd)
{
    ...
    case SNDRV_PCM_TRIGGER_START:
        ...
        wr(e, 0x41018, e->tx_queued & (NC_TX_FRAMES - 1));
        wr(e, DMA_CTL, rd(e, DMA_CTL) | DMA_ENABLE);
        ...
        wr(e, CTL, rd(e, CTL) | STREAM_ENABLE);
        flush(e);
        e->last_poll = e->last_progress = e->last_tx_progress = e->started = ktime_get();
        e->running = true;
        starts++;
        break;
...
static snd_pcm_uframes_t native_pointer(struct snd_pcm_substream *ss)
{
    ...
    mutex_lock(&e->io_lock);
    pos = e->ring.produced % NC_ALSA_FRAMES;
    mutex_unlock(&e->io_lock);
    return pos;
}
...
static int native_poll(void *data)
{
    ...
    struct snd_pcm_substream *ss = e->pcm->streams[SNDRV_PCM_STREAM_CAPTURE].substream;
    while (!kthread_should_stop()) {
        ...
        if (e->running && !e->dead) {
            ...
            u32 tx = rd(e, 0x41058) & (NC_TX_FRAMES - 1);
            u32 rx = rd(e, 0x40818) & (NC_RX_FRAMES - 1);
            u32 tx_n = (tx - e->tx_hw) & (NC_TX_FRAMES - 1);
            u64 before = e->ring.produced;
            e->last_poll = now;
            if (elapsed > max_poll_us) max_poll_us = elapsed;
            if (elapsed >= 80000 || ktime_ms_delta(now, e->last_progress) >= 100 ||
                ktime_ms_delta(now, e->last_tx_progress) >= 100)
                err = -ETIMEDOUT;
            else if (ktime_ms_delta(now, e->started) >= (s64)max_stream_seconds * 1000)
                err = -ETIME;
            else if ((rd(e, DL_STATUS) & 0x101) != 0x101)
                err = -ENOLINK;
            else if (tx_n >= NC_ALSA_FRAMES)
                err = -EPIPE;
            else {
                /* The producer MMIO read precedes payload reads. */
                dma_rmb();
                err = nc_pull(&e->ring, e->rx, ss->runtime->dma_area, rx);
            }
            if (err < 0) {
                last_error = err;
                xruns++;
                dev_err(&e->pdev->dev, "capture failed=%d rx=%u previous=%u produced=%llu taken=%llu gap_us=%lld\n",
                        err, rx, e->ring.hw, e->ring.produced, e->ring.taken, elapsed);
                finish_session(e);
            } else {
                total_frames += e->ring.produced - before;
                ring_wraps += e->ring.produced / NC_RX_FRAMES - before / NC_RX_FRAMES;
                if (e->ring.produced != before) e->last_progress = now;
                /* Replenish independent silence/tone. No input monitoring. */
                nc_tx_fill(e->tx, e->tx_queued, tx_n, loopback);
                e->tx_queued += tx_n;
                e->tx_hw = tx;
                tx_frames += tx_n;
                if (tx_n) e->last_tx_progress = now;
                /* Finish RX reads before returning slots to hardware. */
                dma_mb();
                wr(e, 0x40858, rx);
                wr(e, 0x41018, e->tx_queued & (NC_TX_FRAMES - 1));
                flush(e);
                notify = e->ring.produced / NC_PERIOD != e->notified;
                e->notified = e->ring.produced / NC_PERIOD;
            }
        }
        mutex_unlock(&e->io_lock);
        if (err < 0) snd_pcm_stop(ss, SNDRV_PCM_STATE_XRUN);
...
static int native_probe(struct pci_dev *pdev, const struct pci_device_id *id)
{
    ...
    if (err) {
        dev_err(&pdev->dev, "peripheral preflight failed=%d reg=%02x expected=%x observed=%x\n",
                err, e->peripheral.check_reg, e->peripheral.check_expected, e->peripheral.check_observed);
        goto fail;
    }
    ...
    err = snd_card_new(&pdev->dev, -1, "AvidAdat", THIS_MODULE, 0, &e->card);
    if (err)
        goto fail;
    strscpy(e->card->driver, "AvidAdatTest");
    strscpy(e->card->shortname, "Avid 192 optical pair test");
    snprintf(e->card->longname, sizeof(e->card->longname), "HD Native PCIe + 192 at %s (capture only, experimental)", pci_name(pdev));
    err = snd_pcm_new(e->card, "192 selected optical pair", 0, 0, 1, &e->pcm);
    if (err)
        goto fail;
    e->pcm->private_data = e;
    e->pcm->nonatomic = true;
    strscpy(e->pcm->name, "192 optical pair");
    ...
    if (err)
        goto fail;
    e->worker = kthread_run(native_poll, e, "avid-capture-pcm");
    ...
    bound = true;
    dev_info(&pdev->dev, "ALSA ready: card=%d id=%s; 48 kHz S24_3LE stereo, stream limit=%u s; experimental capture\n",
...
{
    int err;
    if (!enable_experimental || max_stream_seconds != 15 ||
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("HD Native PCIe + 192 bounded ALSA optical pair mapping, 48 kHz stereo");
