...
static void wr(void *p,unsigned off,unsigned cmd) {
    ...
    if(cmd&0x800000) {
        assert((reg==0&&(v==0||v==2))||(reg==0x41&&(v==0||v==9))||(reg==0x4d&&v<=1));
...
static void init(struct sim *s,struct native_192 *n) {
    ...
    *n=(struct native_192){.ctx=s,.read=rd,.write=wr,.now_us=now,.pause=pause_sim,.route_enabled=1,.before=-1,.route_before=-1};
}
static void controls(void) {
    struct sim s;struct native_192 n;unsigned reg,f,v;
    init(&s,&n);assert(!n192_route_enable(&n));assert(s.regs[0x41]==9&&s.regs[0x4d]==1);
    assert(!n192_unmute(&n));assert(!n192_restore(&n));assert(!n192_route_restore(&n));
    assert(s.regs[0]==2&&!s.regs[0x41]&&!s.regs[0x4d]);
    for(reg=0x41;reg<=0x4d;reg+=12) for(f=1;f<=3;f++) {
        init(&s,&n);s.fault_reg=reg;s.fault=f;
        if(f==3) {assert(!n192_route_enable(&n));assert(n192_route_restore(&n));assert(n.route_restore_required);assert(s.off[0x41]==3&&s.off[0x4d]==3);}
        else {assert(n192_route_enable(&n));assert(n.route_restore_required);assert(!n192_route_restore(&n));assert(!s.regs[0x41]&&!s.regs[0x4d]);}
    }
    ...
    puts("PASS controls: both routes, mute, lost ACKs, ignored writes, failed restores, whitelist");
}
...
int main(void){controls();rings();return 0;}
'''
