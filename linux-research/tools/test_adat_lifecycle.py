...
source = (ROOT/'kernel/avid_native_adat.c').read_text()
...
struct native_alsa {
    ...
    struct {unsigned long long produced, taken; unsigned hw;} ring;
    unsigned saved[3];
    void *pdev;
};
...
int main(void) {
    ...
    mute_error=-EBUSY; route_error=-EPROTO;
    assert(finish_session(&e)==0);
    assert(!events[0] && !writes && !resets && !stops && !e.faulted && !last_error);
    assert(finish_session(&e)==0 && !events[0]);
