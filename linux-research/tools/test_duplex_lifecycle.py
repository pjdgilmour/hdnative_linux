...
source = (ROOT/'kernel/avid_native_duplex.c').read_text()
cleanup = source[source.index('static int finish_engine('):source.index('static int configure_engine(')]
prefix = r'''
#include <assert.h>
#include <stdbool.h>
#include <errno.h>
...
enum { PLAY, CAP };
#define TX_BYTES 64
static unsigned stops, playback_stops, capture_stops;
...
struct native_stream {bool active,prepared,notify,xrun;void *area,*runtime;};
struct native_alsa {
    bool running, prepared, configured, quarantined, faulted;
    struct native_192 peripheral;
    struct {unsigned long long played;} play;
...
static struct native_alsa fresh(void) {
    struct native_alsa e={.running=true,.prepared=true,.configured=true,
        ...
        .play={.played=12000},.saved={10,20,30}};
...
int main(void) {
    ...
    assert(finish_engine(&e)==0); /* Route/mute failed before setup_dma. */
    assert(!strcmp(events,"MR") && !writes && stops==0 && resets==1);
    puts("PASS: partial prepare cleans the peripheral without touching DMA");
    e=fresh();
    e.streams[PLAY].active=e.streams[CAP].active=true;
    e.streams[PLAY].prepared=e.streams[CAP].prepared=true;
    memset(e.tx,42,sizeof(e.tx));
    assert(!stop_stream(&e,PLAY));
    assert(e.running && e.configured && e.streams[CAP].active && !writes && !stops);
    assert(!e.streams[PLAY].prepared && last_frames==12000);
    for(unsigned i=0;i<sizeof(e.tx);i++)assert(!e.tx[i]);
    assert(!stop_stream(&e,CAP));assert(!e.running && !e.configured && stops==1);
    puts("PASS: stop playback keeps capture/engine; last close restores once");
    e=fresh(); e.streams[PLAY].active=e.streams[CAP].active=true;
    memset(e.tx,42,sizeof(e.tx));
    assert(!stop_stream(&e,CAP));assert(e.running && e.streams[PLAY].active && !writes);
    assert(e.tx[0]==42);assert(!stop_stream(&e,PLAY));assert(stops==1);
    puts("PASS: stop capture preserves queued playback");
    return 0;
}
'''
