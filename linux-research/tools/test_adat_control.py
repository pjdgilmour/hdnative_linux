...
static unsigned input_pair=1,output_pair=1,optical_path=1;
static unsigned input_selector(void){return (optical_path==1?24:16)+input_pair;}
static unsigned output_reg(void){return (optical_path==1?0x58:0x50)+output_pair;}
...
static void wr(void *p,unsigned off,unsigned cmd) {
    ...
    if(cmd&0x800000) {
        assert((reg==0&&(v==0||v==2))||(reg==0x41&&(v==0||v==input_selector()))||(reg==output_reg()&&v<=1));
...
static void init(struct sim *s,struct native_192 *n) {
    ...
    *n=(struct native_192){.ctx=s,.read=rd,.write=wr,.now_us=now,.pause=pause_sim,.route_enabled=1,.input_pair=input_pair,.output_pair=output_pair,.optical_path=optical_path,.before=-1,.route_before=-1};
}
static void controls(void) {
    struct sim s;struct native_192 n;unsigned reg,f,v;
    init(&s,&n);assert(!n192_route_enable(&n));assert(s.regs[0x41]==input_selector()&&s.regs[output_reg()]==1);
    ...
    init(&s,&n);
    for(reg=0;reg<256;reg++) for(v=0;v<256;v++) {
        unsigned value=v;
        int allowed=(reg==0&&(v==0||v==2)) || (reg==0x41&&(v==0||v==input_selector())) || (reg==output_reg()&&v<=1);
        if(!allowed) {assert(n192_xfer(&n,reg,1,&value)==-EINVAL);assert(!s.tx);}
    }
    for(reg=0x30;reg<=0x32;reg++) {v=0;assert(n192_xfer(&n,reg,0,&v)==-EINVAL);assert(!s.tx);}
    init(&s,&n);s.regs[0x16]=0;assert(n192_route_enable(&n)==-EPROTO);assert(!n.route_write_attempted);
    init(&s,&n);s.regs[1]=1;assert(n192_route_enable(&n)==-EPROTO);assert(!n.route_write_attempted);
...
int main(void){
    for(optical_path=1;optical_path<=2;optical_path++)for(input_pair=1;input_pair<=4;input_pair++)for(output_pair=1;output_pair<=4;output_pair++)controls();
    optical_path=1;input_pair=output_pair=1;
    {struct sim s;struct native_192 n;unsigned v=0;
     init(&s,&n);n.input_pair=0;assert(n192_xfer(&n,0,1,&v)==-EINVAL);assert(!s.tx);
     n.input_pair=1;n.output_pair=5;assert(n192_route_enable(&n)==-EINVAL);assert(!s.tx);
     n.output_pair=1;n.optical_path=0;assert(n192_route_enable(&n)==-EINVAL);assert(!s.tx);
     n.optical_path=3;assert(n192_route_enable(&n)==-EINVAL);assert(!s.tx);}
    puts("PASS 32 optical pair profiles: route allowlist, ACK loss, ignored writes, restoration failures, invalid selections");
    rings();return 0;
}
'''
