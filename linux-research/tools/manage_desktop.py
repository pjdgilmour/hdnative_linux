...
MODULE = 'avid_native_desktop'
BDF = '0000:81:00.0'
PCI = Path('/sys/bus/pci/devices')/BDF
PARAMS = Path('/sys/module')/MODULE/'parameters'
SINK = 'avid_hd_native_192'
RULE_SOURCE = ROOT/'desktop/90-avid-native-desktop.conf'
RULE_TARGET = Path('/etc/wireplumber/wireplumber.conf.d/90-avid-native-desktop.conf')
STATE_DIR = Path('/run/avid-native-desktop')
STATE_FILE = STATE_DIR/'state.json'
MARKER = 'avid-native-desktop-v1'
...
def diagnostics(state):
    ...
    begin=[i for i,s in enumerate(lines) if ': begin desktop ALSA session' in s]
...
def start():
    ...
    if STATE_FILE.exists():
        if PARAMS.exists():
            ...
            status(uid)
            return
        raise RuntimeError('Há estado de uma ativação anterior. Execute desktop-driver.sh stop antes de repetir.')
    if PARAMS.exists() or (PCI/'driver').exists():
        raise RuntimeError('A placa ou o módulo já está em uso fora deste gerenciador.')
    for other in ('avid_native_alsa','avid_native_dma_probe'):
    ...
    build=json.loads((ROOT/'reports/desktop-build.json').read_text())
    ...
    state={'owner':MARKER,'uid':uid,'user':pwd.getpwuid(uid).pw_name,
           ...
           'log_directory':str(ROOT/'reports'/f'desktop-session-{stamp}'),
           'pci_before':header(),'rule_sha256':hashlib.sha256(rule).hexdigest(),
           'rule_owned':not RULE_TARGET.exists(),'default_before':previous,
    ...
    try:
        ...
        while time.monotonic()<deadline:
            ...
            time.sleep(.25)
        if not found:
            raise RuntimeError('O módulo carregou, mas a saída PipeWire não apareceu; veja os logs.')
        ...
        save_state(state)
        diagnostics(state)
        print('DESKTOP_READY=yes')
        print('Selecione “Avid HD Native — 192 I/O (saídas 1–2)” nos aplicativos ou no controle de som.')
        print('Volume inicial -40 dB; ajuste gradualmente pelo controle de som. Reprodução contínua, sem limite de sessão.')
