...
class DesktopManagerTests(unittest.TestCase):
    def setUp(self):
        ...
        m.RULE_SOURCE.write_bytes((ORIGINAL_ROOT/'desktop/90-avid-native-desktop.conf').read_bytes())
        self.ko=root/'kernel/avid_native_desktop.ko';self.ko.write_bytes(b'fake module')
        (root/'reports/desktop-build.json').write_text(json.dumps({'load_files_sha256':{
            'kernel/avid_native_desktop.ko':hashlib.sha256(self.ko.read_bytes()).hexdigest()}}))
        self.wp=True;self.default='yamaha';self.fail_insmod=False;self.fail_rmmod=False
        self.calls=[]
